...
const getRawConfig = (): Record<string, unknown> => {
  ...
  const configs: Record<AppEnv, Record<string, unknown>> = {
    local: localCfg as Record<string, unknown>,
    development: devCfg as Record<string, unknown>,
    production: prodCfg as Record<string, unknown>,
    test: testCfg as Record<string, unknown>,
  };

  const selectedConfig = configs[env];

  return {
    ...(defaultCfg as Record<string, unknown>),
    ...selectedConfig,
  };
};
