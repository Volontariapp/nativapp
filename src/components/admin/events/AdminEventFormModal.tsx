...
export function AdminEventFormModal({
  ...
  onSubmit,
  isLoading,
}: AdminEventFormModalProps): React.JSX.Element {
  const {
    control,
    handleSubmit,
    reset,
  ...
  const submitForm = (data: EventFormData): void => {
    onSubmit({
      title: data.title,
      ...
      tagIds: [],
      startAt: new Date(),
      endAt: new Date(Date.now() + 86400000 * 2), // 2 days from now
    });
  };

  return (
    <AdminModal visible={visible} onClose={handleClose} title="Créer un Événement">
      <Controller
        control={control}
        name="title"
